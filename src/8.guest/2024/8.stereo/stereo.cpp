...
int main()
{
    ...
    if (window != NULL) {
        ...
        std::cout << "Failed to create GLFW window, using fallback without stereo." << std::endl;
...
void scroll_callback(GLFWwindow* window, double xoffset, double yoffset)
{
    camera.ProcessMouseOrbitScroll(static_cast<float>(yoffset));
}
